...
This project follows our [Community Code of Conduct](./CODE_OF_CONDUCT.md) (inspired by Contributor Covenant 3.0). By participating you are expected to uphold it. Report unacceptable behavior to **azelann.borde@gmail.com** — or privately via [Security](#security) advisory if preferred.
...
Browse [good first issues](https://github.com/SirEriTheRed/ts-bastion/labels/good%20first%20issue) and [open milestones](https://github.com/SirEriTheRed/ts-bastion/milestones) to find where to help.

## Getting Started (Development Environment Setup / Local Development Workflow)

```bash
# 1. Clone
git clone https://github.com/SirEriTheRed/ts-bastion.git
cd ts-bastion
...
- Update `package.json` `name` (`@scope/name`)
...
npm run audit:signatures       # strict Sigstore verify (missing/invalid → non-zero)
npm run audit:signatures:warn  # non-blocking wrapper → reports/audit-signatures.txt
zizmor .                 # GitHub Actions audit
zizmor --persona pedantic .
```

CI `audit-signatures` artifact: `reports/audit-signatures.txt` (informational, `continue-on-error: true`, not a required check — see [Security](#security)).
...
- **Questions / Ideas:** [GitHub Discussions](https://github.com/SirEriTheRed/ts-bastion/discussions)
- **Bugs / Features:** [Issue tracker](https://github.com/SirEriTheRed/ts-bastion/issues/new/choose)
...
Distributed under the [MIT License](./LICENSE). By contributing you agree your contributions are licensed under the same terms. See `LICENSE` for details.
